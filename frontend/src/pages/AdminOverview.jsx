        ...
        <div key={i} className="flex-1 flex flex-col items-center gap-1">
          <div className="w-full bg-gray-100 rounded h-full flex items-end">
            <div
              ...
              className="w-full rounded bg-gradient-to-t from-green-600 to-green-400"
            ></div>
          </div>
...
export default function AdminOverview() {
  ...
  const weeklyAppointments = [12, 18, 25, 22, 30, 28, 26];
  const monthlyRevenue = [5.2, 6.1, 7.3, 6.8, 8.0, 7.85];
  const specialityDist = useMemo(() => {
    const counts = doctors.reduce((acc, d) => {
      acc[d.speciality] = (acc[d.speciality] || 0) + 1;
      return acc;
    }, {});
    ...
    <div className="py-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded shadow-sm p-4">
          ...
          {specialityDist.data.length > 0 && (
            <BarChart
              data={specialityDist.data}
              labels={specialityDist.labels}
            />
          )}
        </div>
      </div>
